                    ...
                    <div className="space-y-10">
                        ...
                        <div className="bg-white rounded-[2.5rem] border border-zinc-200 p-10 shadow-sm">
                            ...
                            <div className="space-y-4">
                                ...
                                <button onClick={() => setConfirmModal({ title: targetUser.is_active ? 'Ban Account' : 'Revoke Ban', message: targetUser.is_active ? 'Ban this user account? They will lose access immediately.' : 'Revoke the ban on this account?', onConfirm: () => router.post(route('admin.users.toggle', { user: targetUser.id })), danger: targetUser.is_active })} className={`w-full p-5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all border shadow-sm ${targetUser.is_active ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>
