...
import { Wrench, Search, Loader2, Send, RotateCcw, CheckCircle2, PlayCircle } from 'lucide-react';
...
interface Props {
    ...
    statuses: string[];
    stats: { pending_setup: number; in_progress: number; completed: number; cancelled: number; revenue: number };
}
...
export default function ExtraServicesIndex({ bookings, filters, statuses, stats }: Props) {
    ...
    const go = (params: Record<string, string>) =>
        router.get(route('admin.extra-services.index'), { ...filters, ...params }, { preserveState: true, replace: true });

    const setStatus = (b: Booking, status: string) => {
        setBusy(b.id);
        router.post(route('admin.extra-services.status', b.id), { status }, {
            preserveScroll: true,
    ...
    const refund = (b: Booking) => {
        if (!confirm(`Cancel booking #${b.id} and refund $${b.total} to ${b.user?.name ?? 'the customer'}?`)) return;
        setBusy(b.id);
        router.post(route('admin.extra-services.refund', b.id), {}, {
            preserveScroll: true,
            onFinish: () => setBusy(null),
        });
    };
    ...
    const sendMessage = (b: Booking) => {
        ...
        setBusy(b.id);
        router.post(route('admin.extra-services.message', b.id), { message: draft }, {
            preserveScroll: true,
    ...
    const tiles = [
        ...
        { label: 'In progress', value: stats.in_progress, tone: 'text-indigo-600' },
    ...
    return (
        <AdminLayout>
            <Head title="Account help bookings" />
            <div className="p-6 md:p-8 space-y-8 max-w-7xl mx-auto">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-zinc-900 tracking-tight">Account help bookings</h1>
                        <p className="text-sm text-zinc-500 font-medium">One-off paid support — ad setup & training, account fixes, recovery</p>
                    </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row gap-3 p-4 bg-white rounded-3xl border border-zinc-200">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-3 rounded-2xl bg-zinc-50 border-none font-medium text-zinc-900 focus:ring-2 focus:ring-emerald-500"
                        />
                    </div>
                    <select
                        value={filters.status ?? ''}
                        onChange={e => go({ status: e.target.value })}
                        className="px-4 py-3 rounded-2xl bg-zinc-50 border-none font-bold text-zinc-900 focus:ring-2 focus:ring-emerald-500 cursor-pointer"
                    >
                ...
                <div className="bg-white rounded-2xl border border-zinc-200 overflow-hidden">
                    {bookings.data.length === 0 && (
                        <p className="px-6 py-16 text-center text-zinc-400 font-medium">No account-help bookings yet.</p>
                    )}
                                ...
                                <div className="flex flex-wrap items-start gap-3">
                                    <div className="min-w-0 flex-1">
                                        <div className="flex items-center gap-2 flex-wrap">
                                            ...
                                            <span className="text-xs font-bold text-zinc-500">{label(b.item)}</span>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {b.status === 'pending_setup' && (
                                            <button onClick={() => setStatus(b, 'in_progress')} disabled={busy === b.id}
                                                className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-indigo-500 text-white text-xs font-bold hover:bg-indigo-600 disabled:opacity-50">
                                                {busy === b.id ? <Loader2 size={14} className="animate-spin" /> : <PlayCircle size={14} />} Start
                                            </button>
                                        )}
                                ...
                                {messaging === b.id && (
                                    <div className="flex gap-2 items-end pt-1">
                                        <textarea
                                            ...
                                            className="flex-1 resize-none bg-zinc-50 border-2 border-zinc-100 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-emerald-500"
                                        />
                                        <button onClick={() => sendMessage(b)} disabled={busy === b.id || !draft.trim()}
                                            className="px-4 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-bold hover:bg-emerald-700 disabled:opacity-40">
                                            Send
                                        </button>
                                    </div>
                                )}
                            ...
                            <div className="flex flex-wrap gap-1">
                                {bookings.links.map((l, i) => (
                                    <button key={i} disabled={!l.url}
                                        ...
                                        className={`px-3 py-1.5 rounded-lg text-xs font-bold ${l.active ? 'bg-emerald-600 text-white' : l.url ? 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200' : 'bg-transparent text-zinc-300'}`}
