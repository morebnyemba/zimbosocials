...
class ServiceListFormatter
{
    ...
    public function format(string $rawList, string $platform): ?string
    {
        if (! $this->isAvailable()) {
            return null;
        }
...
STRICT RULES:
- Do NOT invent, omit, reorder within a category, or alter any service name, price, or minimum order value. Copy every name and number EXACTLY as given below, character for character.
...
- Match the formatting conventions and tone typical of {$platform} (e.g. WhatsApp: bold with single *asterisks*, short lines, emojis; Telegram: channel-post style; Twitter/X: compact, punchy, thread-friendly; Instagram: caption style, can include hashtags at the end; Facebook: friendly post style).

Raw service list (source of truth — do not change any values in here):
{$rawList}
